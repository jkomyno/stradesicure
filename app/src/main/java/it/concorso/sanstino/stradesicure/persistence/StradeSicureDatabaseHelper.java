package it.concorso.sanstino.stradesicure.persistence;
...
import it.concorso.sanstino.stradesicure.helper.JsonHelper;
 ...
 */
public class StradeSicureDatabaseHelper extends SQLiteOpenHelper {

    private static final String TAG = "StradeSicureDatabaseHelper";
    private static final String DB_NAME = "stradesicure";
    private static final String DB_SUFFIX = ".db";
    private static final int DB_VERSION = 1;
    private static List<Category> mCategories;
    private static StradeSicureDatabaseHelper mInstance;
    private final Resources mResources;

    private StradeSicureDatabaseHelper(Context context) {
        //prevents external instance creation
        super(context, DB_NAME + DB_SUFFIX, null, DB_VERSION);
        mResources = context.getResources();
    }

    private static StradeSicureDatabaseHelper getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new StradeSicureDatabaseHelper(context.getApplicationContext());
        }
        return mInstance;
    }
    ...
    private static List<Category> loadCategories(Context context) {
        Cursor data = StradeSicureDatabaseHelper.getCategoryCursor(context);
        List<Category> tmpCategories = new ArrayList<>(data.getCount());
        final SQLiteDatabase readableDatabase = StradeSicureDatabaseHelper.getReadableDatabase(context);
        do {
    ...
    /**
     * Resets the contents of StradeSicure's database to its initial state.
     *
     * @param context The context this is running in.
     */
    ...
    private static List<Quiz> getQuizzes(final String categoryId, SQLiteDatabase database) {
        ...
        // tableName, tableColumns, whereClause, whereArgs, groupBy, having, orderBy, limit
        final Cursor cursor = database.query(QuizTable.NAME, QuizTable.PROJECTION,
                QuizTable.FK_CATEGORY + " LIKE ?", new String[]{categoryId}, null, null, "RANDOM()", "10");
        cursor.moveToFirst();
        do {
    ...
    public void onCreate(SQLiteDatabase db) {
         ...
         */
        Log.d("Helper", "onCreatePersistence");
